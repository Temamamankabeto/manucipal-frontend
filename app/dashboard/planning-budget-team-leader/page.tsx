...
export default function PlanningBudgetTeamLeaderDashboardPage() {
  ...
  const paymentFirstReview = statusCount(payments, ["budget_tl_review"]);
  const paymentFinalReview = statusCount(payments, ["budget_tl_final_review"]);
  const paymentExpertProcessing = statusCount(payments, ["budget_expert_processing"]);
  const paymentCompleted = statusCount(payments, ["payment_completed"]);

  const procurementBudgetReview = statusCount(procurements, ["budget_tl_review"]);
  const procurementExpertProcessing = statusCount(procurements, ["budget_expert_processing"]);
  const procurementCompleted = statusCount(procurements, ["completed"]);

  const totalWorkflows = payments.length + procurements.length;
  const activeBudgetReviews =
    paymentFirstReview +
    paymentFinalReview +
    paymentExpertProcessing +
    procurementBudgetReview +
    procurementExpertProcessing;
  ...
  const paymentStatusRows = [
    { label: "Budget TL review", value: paymentFirstReview },
    { label: "Expert processing", value: paymentExpertProcessing },
    { label: "Final review", value: paymentFinalReview },
    { label: "Completed", value: paymentCompleted },
  ];

  const procurementStatusRows = [
    { label: "Budget TL review", value: procurementBudgetReview },
    { label: "Expert processing", value: procurementExpertProcessing },
    ...
    <div className="space-y-6">
      <div className="relative overflow-hidden rounded-3xl border bg-card p-6 shadow-sm">
        ...
        <div className="relative flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <Badge className="mb-3 rounded-full">Planning & Budget Team Leader</Badge>
            <h1 className="text-2xl font-bold tracking-tight md:text-3xl">
              Budget Review Command Dashboard
            </h1>
            <p className="mt-2 max-w-3xl text-sm text-muted-foreground">
              Track payment and procurement budget reviews, expert processing, final approvals, and reporting from one Power BI-style workspace.
            </p>
          </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <DashboardStat
          title="Active budget reviews"
          value={activeBudgetReviews}
          subtitle="Payments and procurements awaiting budget action"
          icon={ReceiptText}
        />
        <DashboardStat
          title="Payment queue"
          value={paymentFirstReview + paymentFinalReview}
          subtitle={`${paymentExpertProcessing} with budget experts`}
          icon={CreditCard}
        />
        <DashboardStat
          title="Procurement queue"
          value={procurementBudgetReview}
          subtitle={`${procurementExpertProcessing} with budget experts`}
          icon={PackageCheck}
        />
        <DashboardStat
          title="Financial value"
          value={formatMoney(totalFinancialValue)}
          subtitle="Payments plus procurement estimates"
          icon={TrendingUp}
        />
      </div>
        ...
        <Card className="rounded-2xl border bg-card shadow-sm">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-base font-semibold">
              <BarChart3 className="h-5 w-5 text-primary" /> Budget workflow performance
            </CardTitle>
          </CardHeader>
            ...
            <div className="grid gap-4 md:grid-cols-3">
              <div className="rounded-2xl border bg-background p-4">
                ...
                <p className="mt-2 text-3xl font-bold">
                  {percent(completedWorkflows, totalWorkflows)}%
                </p>
                <Progress value={percent(completedWorkflows, totalWorkflows)} className="mt-4 h-2" />
              </div>
              <div className="rounded-2xl border bg-background p-4">
                <p className="text-sm text-muted-foreground">Active review rate</p>
                <p className="mt-2 text-3xl font-bold">
                  {percent(activeBudgetReviews, totalWorkflows)}%
                </p>
                <Progress value={percent(activeBudgetReviews, totalWorkflows)} className="mt-4 h-2" />
              </div>
              <div className="rounded-2xl border bg-background p-4">
                <p className="text-sm text-muted-foreground">Total workflows</p>
                <p className="mt-2 text-3xl font-bold">{totalWorkflows}</p>
                <p className="mt-4 text-xs text-muted-foreground">Payment and procurement requests</p>
              </div>
            </div>
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <div className="space-y-4">
                <div className="flex items-center gap-2 font-semibold">
                  <CreditCard className="h-4 w-4 text-primary" /> Payment review status
                </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2 font-semibold">
                  <ShoppingCart className="h-4 w-4 text-primary" /> Procurement review status
                </div>
        ...
        <Card className="rounded-2xl border bg-card shadow-sm">
          ...
          <CardContent className="space-y-3">
            <Button asChild variant="outline" className="w-full justify-start">
              <Link href="/dashboard/payment">
                <CreditCard className="mr-2 h-4 w-4" /> Open payment reviews
              </Link>
            </Button>
            <Button asChild variant="outline" className="w-full justify-start">
              <Link href="/dashboard/procurement">
                <ShoppingCart className="mr-2 h-4 w-4" /> Open procurement reviews
              </Link>
            </Button>
            <Button asChild variant="outline" className="w-full justify-start">
              <Link href="/dashboard/reports/procurement-payment?tab=payment">
                <FileText className="mr-2 h-4 w-4" /> Payment report
              </Link>
            </Button>
            <Button asChild variant="outline" className="w-full justify-start">
              <Link href="/dashboard/reports/procurement-payment?tab=procurement">
                <FileText className="mr-2 h-4 w-4" /> Procurement report
              </Link>
            </Button>
            <div className="rounded-2xl border bg-muted/40 p-4">
              <div className="flex items-center gap-2 text-sm font-semibold">
                <CheckCircle2 className="h-4 w-4 text-primary" /> Budget workflow scope
              </div>
              <p className="mt-2 text-xs text-muted-foreground">
                This dashboard focuses on budget team leader review, expert processing, final payment review, and report access.
              </p>
            </div>
          </CardContent>
        </Card>
