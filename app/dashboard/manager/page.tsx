...
import { useUsersQuery } from "@/hooks/user/use-users";
...
export default function ManagerDashboardPage() {
  ...
  const paymentStatusRows = [
    ...
    { label: "Draft", value: statusCount(payments, ["draft"]) },
  ];
    ...
    <div className="space-y-6">
      <div className="relative overflow-hidden rounded-3xl border bg-card p-6 shadow-sm">
        ...
        <div className="relative flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <Badge className="mb-3 rounded-full">Manager Workspace</Badge>
            <h1 className="text-2xl font-bold tracking-tight md:text-3xl">
              Procurement & Payment Command Dashboard
            </h1>
            <p className="mt-2 max-w-3xl text-sm text-muted-foreground">
              Monitor payments, procurements, users, approvals, and operational reports from one professional workspace.
            </p>
          </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <DashboardStat
          title="Managed users"
          value={users.length}
          subtitle="Users visible under manager scope"
          icon={Users}
        />
      </div>
          ...
          <CardContent className="space-y-3">
            ...
            <div className="rounded-2xl border bg-muted/40 p-4">
              <div className="flex items-center gap-2 text-sm font-semibold">
                <CheckCircle2 className="h-4 w-4 text-primary" /> Scope protected
              </div>
              <p className="mt-2 text-xs text-muted-foreground">
                Manager dashboard shows only records allowed by backend permissions and manager ownership rules.
              </p>
            </div>
          </CardContent>
        </Card>
